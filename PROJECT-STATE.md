...
| 2 | KB health checker | **v1 BUILT 2026-08-23 — awaiting Aneesh's publish** (flagship) | Paste-mode, pure client-side, verified headless. Target repo `neeshykha/kb-health-checker` + Pages. v2 = URL-fetch mode on Vercel + serverless (CORS) in attended sessions; reconcile rubric with `specs/kb-health-checker-spec.md` from hub bundle. |
